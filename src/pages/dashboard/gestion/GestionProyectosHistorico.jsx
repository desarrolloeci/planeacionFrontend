...
const GestionProyectosHistorico = () => {
  ...
  const proyectosFiltrados = proyectos.filter(p =>
    ...
    (criterios.unidadejecutora === '' || p.unidadejecutora === String(criterios.unidadejecutora)) &&
    (criterios.estado === '' || p.estadopr === Number(criterios.estado)) &&
    (criterios.fechacrea === '' || p.fechacrea === criterios.fechacrea)
  );
  ...
  const handleDetalle = async (id) => {
    try {
      ...
      setStorage("proyectoParaEditar", JSON.stringify(dummyProyecto));
      navigate(paths.dashboard.gestion.detalleProyecto);
    ...
    try {
      ...
      if (esGestionador) {
        data2 = data2.filter(p =>
          p.ccdirectorpr === usuario.id || p.ccresponsablepr === usuario.id
        );
      }
      ...
      console.error("Error al cargar proyectos desde JWT:", error);
    }
  };
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        heading="Historico de proyectos"
        links={[
          { name: 'Dashboard', href: paths.dashboard.root },
          { name: 'Historico de proyectos' },
        ]}
        action={
          <Button
          ...
          <Table stickyHeader>
            ...
            <TableBody>
              {sortedProyectos
                ...
                .map((proyecto) => (
                  <TableRow key={proyecto.idproyecto} hover>
                    ...
                    <TableCell>{unidades.find(p => String(p.id) === String(proyecto.unidadejecutora))?.name || 'N/A'}</TableCell>
                    <TableCell>{estados.find(p => String(p.id) === String(proyecto.estadopr))?.name || 'N/A'}</TableCell>
                    <TableCell>{proyecto.fechacrea}</TableCell>
                    <TableCell>
                      <IconButton color="secondary" onClick={() => handleDetalle(proyecto.idproyecto)}>
              ...
              {proyectosFiltrados.length === 0 && (
                <TableRow>
                  <TableCell colSpan={7} align="center">No se encontraron proyectos</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
...
export default GestionProyectosHistorico;
