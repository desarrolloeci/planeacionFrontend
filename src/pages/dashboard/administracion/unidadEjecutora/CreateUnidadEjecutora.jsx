...
const metadata = { title: `Crear Unidad Ejecutora | Dashboard - ${CONFIG.appName}` };

export default function CreateUnidadEjecutora() {
    ...
    const onSubmit = handleSubmit(async (data) => {
        ...
        const raw = JSON.stringify({
            "nombreunidad": data.name,
            "estadounidadej": data.state === true ? 1 : 0
        });
        ...
        fetch(`${API_BASE_URL}/unidadejecutora`, requestOptions)
            .then((response) => response.text())
            .then((result) => {
                toast.success('Se creo correctamente el registro!');
                navigate(paths.dashboard.administracion.rootUnidadEjecutora)
            })
            .catch((error) => {
    ...
    return (
        <>
            ...
            <DashboardContent sx={{ width: "100%" }}>
                <CustomBreadcrumbs
                    heading="Crear unidad ejecutora"
                    backHref={paths.dashboard.administracion.rootUnidadEjecutora}
                    links={[
                        { name: 'Dashboard', href: paths.dashboard.root },
                        { name: 'Unidad ejecutora', href: paths.dashboard.administracion.rootUnidadEjecutora },
                        { name: 'Nueva unidad ejecutora' },
                    ]}
                    sx={{ mb: { xs: 3, md: 5 } }}
                />
