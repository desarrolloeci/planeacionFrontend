...
const metadata = { title: `Fin de Autoevaluación | Dashboard - ${CONFIG.appName}` };

export default function FinAutoevaluacion() {
    ...
    const columns = [
        ...
        { field: 'secuencial', headerName: 'Objetivo', flex: 1 },
        {
            field: 'actions',
            type: 'actions',
    ...
    const actualizar = (row) => {
        ...
        navigate(paths.dashboard.administracion.editFinAutoevaluacion(row.id))
    ...
    return (
        <>
            ...
            <DashboardContent>
                <CustomBreadcrumbs
                    heading="Fin Autoevaluación"
                    links={[
                        { name: 'Dashboard', href: paths.dashboard.root },
                        { name: 'Fin de autoevaluación' },
                    ]}
                    action={
                        <Button
                            component={RouterLink}
                            href={paths.dashboard.administracion.newFinAutoevaluacion}
                            variant="contained"
                            color="primary"
                            startIcon={<Iconify icon="mingcute:add-line" />}
                        >
                            Nuevo fin autoevaluacion
                        </Button>
                    }
                    sx={{ mb: { xs: 3, md: 5 } }}
                />
